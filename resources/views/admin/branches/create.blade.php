...
<div class="container-fluid px-4 py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="h3 mb-0 text-gray-800">Add Branch</h2>
            ...
            <form action="{{ route('admin.branches.store') }}" method="POST">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control @error('code') is-invalid @enderror" id="code" name="code" value="{{ old('code') }}" placeholder="Enter branch code" required>
                        @error('code')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-4">
                        ...
                        <select class="form-select @error('branch_type') is-invalid @enderror" id="branch_type" name="branch_type" required>
                            <option value="training_center" {{ old('branch_type') == 'training_center' ? 'selected' : '' }}>Training Center</option>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('address') is-invalid @enderror" id="address" name="address" rows="2" placeholder="Enter branch address">{{ old('address') }}</textarea>
                    @error('address')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="d-flex justify-content-end">
                    <button type="submit" class="btn btn-primary px-4 shadow-sm">Save Branch</button>
                </div>
            </form>
        </div>
    </div>
