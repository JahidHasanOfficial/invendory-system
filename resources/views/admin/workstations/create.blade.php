...
@section('title', 'Add Workstation')

@section('content')
<div class="container-fluid px-4 py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="h3 mb-0 text-gray-800">Add Workstation</h2>
        ...
        <div class="card-body">
            <form action="{{ route('admin.workstations.store') }}" method="POST">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('lab_id') is-invalid @enderror" id="lab_id" name="lab_id" required>
                            ...
                            @foreach($labs as $lab)
                                <option value="{{ $lab->id }}" {{ old('lab_id') == $lab->id ? 'selected' : '' }}>{{ $lab->name }} ({{ $lab->branch ? $lab->branch->name : '' }})</option>
                            @endforeach
                        </select>
                        @error('lab_id')
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control @error('workstation_code') is-invalid @enderror" id="workstation_code" name="workstation_code" value="{{ old('workstation_code') }}" placeholder="Enter workstation code" required>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <select class="form-select @error('workstation_type') is-invalid @enderror" id="workstation_type" name="workstation_type" required>
                            <option value="student" {{ old('workstation_type') == 'student' ? 'selected' : '' }}>Student</option>
                            <option value="instructor" {{ old('workstation_type') == 'instructor' ? 'selected' : '' }}>Instructor</option>
                            <option value="server" {{ old('workstation_type') == 'server' ? 'selected' : '' }}>Server</option>
                        </select>
                ...
                <div class="d-flex justify-content-end">
                    <button type="submit" class="btn btn-primary px-4 shadow-sm">Save Workstation</button>
                </div>
            </form>
        </div>
    </div>
