...
<div class="container-fluid px-4 py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="h3 mb-0 text-gray-800">Edit Asset Assignment</h2>
            ...
            <form action="{{ route('admin.assets.update', $asset->id) }}" method="POST">
                ...
                <div class="row mb-3">
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control" value="{{ $asset->product ? $asset->product->name : '' }}" disabled>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control @error('serial_no') is-invalid @enderror" id="serial_no" name="serial_no" value="{{ old('serial_no', $asset->serial_no) }}" placeholder="Enter serial number" required>
                        @error('serial_no')
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select @error('branch_id') is-invalid @enderror" id="branch_id" name="branch_id" required>
                            @foreach($branches as $branch)
                                <option value="{{ $branch->id }}" {{ old('branch_id', $asset->branch_id) == $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                            @endforeach
                        </select>
                        @error('branch_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="row mb-3">
                    <div class="col-md-4">
                        ...
                        <select class="form-select @error('lab_id') is-invalid @enderror" id="lab_id" name="lab_id">
                            ...
                            @foreach($labs as $lab)
                                <option value="{{ $lab->id }}" {{ old('lab_id', $asset->lab_id) == $lab->id ? 'selected' : '' }}>{{ $lab->name }}</option>
                            @endforeach
                        </select>
                        @error('lab_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select @error('assigned_to_user_id') is-invalid @enderror" id="assigned_to_user_id" name="assigned_to_user_id">
                            ...
                            @foreach($users as $user)
                                <option value="{{ $user->id }}" {{ old('assigned_to_user_id', $asset->assigned_to_user_id) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select @error('assignment_type') is-invalid @enderror" id="assignment_type" name="assignment_type" required>
                            <option value="permanent" {{ old('assignment_type', $asset->assignment_type) == 'permanent' ? 'selected' : '' }}>Permanent</option>
                            <option value="temporary" {{ old('assignment_type', $asset->assignment_type) == 'temporary' ? 'selected' : '' }}>Temporary</option>
                            <option value="lab_assigned" {{ old('assignment_type', $asset->assignment_type) == 'lab_assigned' ? 'selected' : '' }}>Lab Assigned</option>
                        </select>
                ...
                <div class="row mb-3">
                    <div class="col-md-4">
                        ...
                        <input type="date" class="form-control @error('assigned_date') is-invalid @enderror" id="assigned_date" name="assigned_date" value="{{ old('assigned_date', $asset->assigned_date) }}" required>
                        @error('assigned_date')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-4">
                        ...
                        <select class="form-select @error('condition') is-invalid @enderror" id="condition" name="condition" required>
                            ...
                            <option value="under_repair" {{ old('condition', $asset->condition) == 'under_repair' ? 'selected' : '' }}>Under Repair</option>
                        </select>
                        @error('condition')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="mb-4">
                    ...
                    <textarea class="form-control @error('notes') is-invalid @enderror" id="notes" name="notes" rows="2" placeholder="Enter notes">{{ old('notes', $asset->notes) }}</textarea>
                    @error('notes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="d-flex justify-content-end">
                    <button type="submit" class="btn btn-primary px-4 shadow-sm">Update Assignment</button>
                </div>
            </form>
        </div>
    </div>
