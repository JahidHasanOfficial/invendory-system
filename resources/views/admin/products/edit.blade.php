...
<div class="container-fluid px-4 py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="h3 mb-0 text-gray-800">Edit Product</h2>
            ...
            <div class="card-body">
                ...
                <div class="row mb-3">
                    <div class="col-md-3">
                        ...
                        <select class="form-select @error('unit_id') is-invalid @enderror" id="unit_id" name="unit_id">
                            ...
                            @foreach($units as $unit)
                                <option value="{{ $unit->id }}" {{ old('unit_id', $product->unit_id) == $unit->id ? 'selected' : '' }}>{{ $unit->full_name }} ({{ $unit->short_name }})</option>
                            @endforeach
                        </select>
                        @error('unit_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="number" step="0.01" class="form-control @error('purchase_price') is-invalid @enderror" id="purchase_price" name="purchase_price" value="{{ old('purchase_price', $product->purchase_price) }}">
        ...
        <div class="card shadow mb-4 border-0">
            ...
            <div class="card-body">
                <div class="row mb-4">
                    <div class="col-md-3">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" id="is_batch_tracked" name="is_batch_tracked" value="1" {{ old('is_batch_tracked', $product->is_batch_tracked) ? 'checked' : '' }}>
                            <label class="form-check-label" for="is_batch_tracked">Batch Tracked</label>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" id="is_serial_tracked" name="is_serial_tracked" value="1" {{ old('is_serial_tracked', $product->is_serial_tracked) ? 'checked' : '' }}>
                            <label class="form-check-label" for="is_serial_tracked">Serial Tracked</label>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" id="is_asset" name="is_asset" value="1" {{ old('is_asset', $product->is_asset) ? 'checked' : '' }}>
                ...
                <div class="d-flex justify-content-end mt-4">
                    <button type="submit" class="btn btn-primary px-4 shadow-sm">Update Product</button>
                </div>
            </div>
        </div>
    </form>
